...
平衡模式含 Planner、Blind Reader 和 State Settler 在内最多创建 12 个 Agent 任务，最大并发固定为 2；快速模式在盲读与状态证据门禁开启时最多创建 5 个任务（规划、单候选、集成初筛、盲读、状态结算），始终人工确认。失败、超时和无效输出均计入本次任务上限；达到上限后停止创建新任务，保留现有工件并进入 `WAITING_USER`。sub-agent 和顺序执行都必须使用相同任务与工件协议。
...
- `READY` 要求总分至少 85、连续性至少 90、其余单项至少 82；接近候选需一次有效盲选，单合格候选需额外连续性专项守卫。
- 总分至少 78 但未满足全部 `READY` 条件时进入人工确认；正式状态保持不变。
...
阶段任务统一使用 `creative-task/1` 与 `creative-result/1` 工件；Python 只验证这些工件的结构、指纹和正文证据。
...
规划、写作、语义审查、盲读和阶段复审不是命令行功能。告诉当前 Agent 阅读 `WORKFLOW.md` 并执行相应工作流；Python 只做确定性校验和事务操作，不启动模型。
...
5. Agent 任务中断时，重新读取对应任务信封和运行清单；只有正式状态、输入和工件指纹都匹配才可复用。
6. 恢复后人工检查 `decision.md`，再执行 `accept` 或选择明确的合格候选。
7. `WAITING_USER` 可在不创建新任务的情况下检查最佳工件；需要新增生成或审查时，显式建立新的任务信封。
