...
| 能力 | 作用 |
|---|---|
| 质量进化分工 | Planner 先建立戏剧脊柱再维护完整约束账本，Writer 只读取精简故事简报，Reviewer 使用完整账本审计 |
| 单一真相源 | 当前章号、人物、时间、伏笔和已知规则集中在一个状态文件中 |
| 事务式写作 | 章节、元数据和状态同步提升；任一步骤失败都不改变正式内容 |
| 多层质量门禁 | 四维计分后执行无大纲盲读，检查开场抓力、主角行动权、问题升级、章末追读力和首个弃读点 |
...
- 状态结构、当前事件和正式章号连续；
- 规划引用的事实 ID 已注册，并产生实际状态变化；规划还必须明确开场压力与类型信号、主角即时欲望、亲自选择、由本人实际支付的代价、关系摩擦、非重复的问题升级、连载承诺及章末行动的真实制约力；
- 未解锁能力、现代人物和时代错位词汇没有进入正文；
- 最近五章不存在完整段落复用、累计长句复用或明显场景模板复制；
- 集成初筛及实际触发的专项证据确实存在于正文，各维度达到资格线；
- 双候选接近时一次匿名盲选有效，单合格候选还必须通过连续性专项守卫；
- 无大纲盲读的八项体验分都达到 4/5，至少接近同类型优秀网文开篇，并给出具体的下一章点击理由；
